...
export function calculateDailyEmployeeCapacity(employee, capacityRules = []) {
  ...
  // Fall back to default capacity rules if user-configured rules don't exist for this role
  const rulesToUse = roleRules.length > 0
    ? roleRules
    : DEFAULT_CAPACITY_RULES.filter(r => normalizeRole(r.role) === role);

  if (rulesToUse.length === 0) {
    // Standard default: 7 units/day for Graphic Designer, 4 units/day for Video Editor
    return role === ROLES.VIDEO_EDITOR ? 4 : 7;
  }

  const totalDailyUnits = rulesToUse.reduce((sum, rule) => {
    const qty = Number(rule.dailyQuantity) || 0;
    const weight = Number(rule.capacityWeight) || 1;
    return sum + (qty * weight);
  }, 0);

  return totalDailyUnits > 0 ? totalDailyUnits : (role === ROLES.VIDEO_EDITOR ? 4 : 7);
}
