...
export function exportAgencyMatrixGridExcel({
  ...
  const weekTitle = title || (monthLabel ? `${monthLabel} (Full Month Plan)` : (week?.name || 'Weekly Plan'));
    ...
    <table>
      <thead>
        <tr>
          ...
          <th class="header-yellow" style="width: 220px; white-space: nowrap;">${displayHeader}</th>
          ${staffHeadersHtml}
  ...
  const cleanFilename = `Agency_Work_Distribution_${(week?.name || dayName || 'Week').replace(/\s+/g, '_')}`;
  downloadExcelHTML(cleanFilename, html);
}

/**
 * Export full clean weekly allocation report with Matrix only.
 */
