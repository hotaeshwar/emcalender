...
export function exportAgencyMatrixGridExcel({
  ...
  // Build matrix data map: clientMap[clientId][employeeId] = { posts, reels, stories }
  const matrixMap = {};
  ...
  const rowsHtml = clientList.map((client, cIdx) => {
    ...
    return `
      <tr>
        ...
        ${staffCellsHtml}
        <td class="cell-total">${clientPosts > 0 ? clientPosts : ''}</td>
        <td class="cell-total">${clientReels > 0 ? clientReels : ''}</td>
        <td class="cell-total">${clientStories > 0 ? clientStories : ''}</td>
        <td class="cell-total td-bold">${clientWeekCount > 0 ? clientWeekCount : ''}</td>
      </tr>
    `;
  }).join('');
  ...
  const html = `
    <div className="title-banner" style="margin-bottom: 10px;">Bid Employee Work Distributer</div>
