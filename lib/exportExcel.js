...
export function exportAgencyMatrixGridExcel({
  ...
  // Staff List strictly from active Firebase employees
  ...
  // Client List strictly from active Firebase clients
  ...
  if (staffList.length === 0 || clientList.length === 0) {
    alert('Cannot export Excel matrix: Employees or Clients list is empty in database.');
    return;
  }

  const activeAllocations = Array.isArray(allocations) ? allocations : [];

  // Multi-Key Indexing Map
  const matrixMap = {};
  ...
  staffList.forEach((s, idx) => {
    ...
    staffHeadersHtml += `<th colspan="3" class="${bgClass}" style="white-space: nowrap; text-align: center; font-weight: bold;">${s.name?.toUpperCase()}</th>`;
  });
  ...
  const html = `
    ...
    <table>
      <thead>
        <tr>
          <th rowspan="2" style="width: 50px; white-space: nowrap;">S.NO</th>
          <th class="header-yellow" style="width: 220px; white-space: nowrap;">${dayName.toUpperCase()}</th>
          ${staffHeadersHtml}
          <th colspan="3" class="header-total" style="white-space: nowrap;">TOTAL COUNT</th>
          <th rowspan="2" class="header-total" style="width: 100px; white-space: nowrap;">WEEK COUNT</th>
        </tr>
        <tr>
          <th class="header-clients" style="white-space: nowrap;">CLIENTS</th>
          ${staffSubHeadersHtml}
...
/**
 * Export full color-coded weekly allocation report with Matrix, Day-by-Day Timetable, and Surplus Work.
 */
export function exportAllocationReport({
  ...
  // Section 2: Day-Wise Production Schedule
  let dayScheduleSectionHtml = '';
  const workingDates = Array.isArray(week?.workingDates) ? week.workingDates : [];
  if (workingDates.length > 0) {
    const dayHeaders = workingDates.map(d => `
      <th style="background-color: #1e3a8a; color: #ffffff; text-align: center; font-size: 9pt;">
        ${formatWeekday(d).toUpperCase()}<br/><span style="font-size: 8pt; font-weight: normal; color: #93c5fd;">${d}</span>
      </th>
    `).join('');

    const dayRows = staffList.map(s => {
      const sKey = (s.name || s.id).toUpperCase();
      const tot = staffTotals[sKey] || { posts: 0, reels: 0, stories: 0 };
      const roleLabel = s.role === 'video_editor' ? 'Video Editor' : 'Graphic Designer';
      
      const dayCells = workingDates.map(d => {
        const isVE = s.role === 'video_editor';
        const pShare = Math.round(tot.posts / workingDates.length);
        const rShare = Math.round(tot.reels / workingDates.length);
        const sShare = Math.round(tot.stories / workingDates.length);
        return `<td style="text-align: center; font-size: 9pt; background-color: #f8fafc;">${isVE ? `${rShare}R ${sShare}S` : `${pShare}P ${rShare}R`}</td>`;
      }).join('');

      return `
        <tr>
          <td style="font-weight: bold; text-align: left; background-color: #f1f5f9;">${s.name} (${s.employeeCode})</td>
          <td style="text-align: center; background-color: #f8fafc;">${roleLabel}</td>
          <td style="text-align: center; font-weight: bold; background-color: #e0e7ff;">${tot.posts}P, ${tot.reels}R, ${tot.stories}S</td>
          ${dayCells}
        </tr>
      `;
    }).join('');

    dayScheduleSectionHtml = `
      <div class="section-header" style="background-color: #1e3a8a; margin-top: 25px;">
        2. Day-by-Day Production Timetable (Monday through Saturday Deliverable Flow)
      </div>
      <table>
        <thead>
          <tr>
            <th style="background-color: #0f172a; color: #ffffff; text-align: left; width: 180px;">Staff Member</th>
            <th style="background-color: #0f172a; color: #ffffff; text-align: center; width: 120px;">Role</th>
            <th style="background-color: #312e81; color: #ffffff; text-align: center; width: 120px;">Weekly Total</th>
            ${dayHeaders}
          </tr>
        </thead>
        <tbody>
  ...
  // Section 3: Surplus Deliverables Log
  let surplusSectionHtml = '';
  if (Array.isArray(surplus) && surplus.length > 0) {
    const surplusRows = surplus.map((s, idx) => `
      <tr class="surplus-row">
        <td>${idx + 1}</td>
        <td style="font-weight: bold; text-align: left;">${s.clientName || s.clientId}</td>
        <td>${(s.contentType || '').toUpperCase()}</td>
        <td>${s.roleRequired === 'video_editor' ? 'Video Editor' : 'Graphic Designer'}</td>
        <td style="font-weight: bold; color: #dc2626; text-align: center;">${s.quantity}</td>
        <td style="text-align: left; font-size: 9pt;">${s.reasonLabel || s.reason || 'Capacity exceeded'}</td>
      </tr>
    `).join('');

    surplusSectionHtml = `
      <div class="section-header" style="background-color: #991b1b; margin-top: 25px;">
        3. Surplus Deliverables (Requiring Manual Staff Distribution)
      </div>
      <table>
        <thead>
          <tr>
            <th style="background-color: #7f1d1d; color: white; width: 40px;">#</th>
            <th style="background-color: #7f1d1d; color: white; text-align: left;">Client</th>
            <th style="background-color: #7f1d1d; color: white;">Content Type</th>
            <th style="background-color: #7f1d1d; color: white;">Role Required</th>
            <th style="background-color: #7f1d1d; color: white; text-align: center;">Surplus Qty</th>
            <th style="background-color: #7f1d1d; color: white; text-align: left;">Surplus Reason</th>
          </tr>
        </thead>
        <tbody>
          ${surplusRows}
  ...
  const html = `
    <div class="title-banner" style="margin-bottom: 5px;">Bid Employee Work Distributer</div>
    <div class="subtitle-banner" style="margin-bottom: 12px;">Operational Plan: ${weekTitle}</div>
    
    <div class="section-header" style="background-color: #4338ca; margin-top: 5px;">
      1. Agency Work Distribution Matrix (${weekTitle})
    </div>
    ...
    <table>
      <thead>
        <tr>
          <th rowspan="2" style="width: 50px; white-space: nowrap;">S.NO</th>
          <th class="header-yellow" style="width: 220px; white-space: nowrap;">MONDAY</th>
          ${staffHeadersHtml}
          <th colspan="3" class="header-total" style="white-space: nowrap;">TOTAL COUNT</th>
          <th rowspan="2" class="header-total" style="width: 100px; white-space: nowrap;">WEEK COUNT</th>
        </tr>
  ...
  downloadExcelHTML(`Agency_Work_Distribution_Plan_${(week?.name || 'Week').replace(/\s+/g, '_')}`, html);
}
