...
const DEFAULT_SEEDS = {
  clients: [
    { id: 'c1', name: 'ACTION CAR DETAILING', clientCode: 'ACD', status: 'active' },
    { id: 'c2', name: 'CHUTNEY HOUSE', clientCode: 'CH', status: 'active' },
    { id: 'c3', name: 'DND', clientCode: 'DND', status: 'active' },
    { id: 'c4', name: 'DIVINE DWELLING', clientCode: 'DD', status: 'active' },
    { id: 'c5', name: 'DEVINE STUDIO', clientCode: 'DS', status: 'active' },
    { id: 'c6', name: 'ISHA INTERNATIONAL', clientCode: 'II', status: 'active' },
    { id: 'c7', name: 'BALAJI EV', clientCode: 'BEV', status: 'active' },
    { id: 'c8', name: 'KC CROSSROAD', clientCode: 'KCC', status: 'active' },
    { id: 'c9', name: 'THE RADIANT MANALI', clientCode: 'TRM', status: 'active' },
    { id: 'c10', name: 'OREN KASAULI', clientCode: 'OK', status: 'active' },
    { id: 'c11', name: 'CELESTIAL TRADER', clientCode: 'CT', status: 'active' },
    { id: 'c12', name: 'TSS', clientCode: 'TSS', status: 'active' },
  ],
  employees: [
    { id: 'emp1', name: 'HARSHITA', employeeCode: 'EMP001', role: 'graphic_designer', status: 'active' },
    { id: 'emp2', name: 'NEHA', employeeCode: 'EMP002', role: 'graphic_designer', status: 'active' },
    { id: 'emp3', name: 'GURJEET', employeeCode: 'EMP003', role: 'video_editor', status: 'active' },
    { id: 'emp4', name: 'KARAN', employeeCode: 'EMP004', role: 'video_editor', status: 'active' },
  ],
  workWeeks: [
    {
      id: 'week_1',
      ...
      endDate: '2026-09-07',
      workingDates: ['2026-09-01', '2026-09-02', '2026-09-03', '2026-09-04', '2026-09-05', '2026-09-07'],
      holidays: [],
      status: 'active',
...
function getLocal(key) {
  ...
  try {
    ...
    // Return seeds if local is empty
    if (DEFAULT_SEEDS[key]) {
      localStorage.setItem(LOCAL_STORAGE_PREFIX + key, JSON.stringify(DEFAULT_SEEDS[key]));
...
export function subscribeCollection(collectionName, callback, sortFn = null) {
  // 1. Immediately provide cached/seeded data to eliminate any loading wait
  let localData = getLocal(collectionName);
  if (!localData || localData.length === 0) {
    localData = DEFAULT_SEEDS[collectionName] || [];
  }
    ...
    unsubscribeFirestore = onSnapshot(
      ...
      (snapshot) => {
        if (!snapshot.empty) {
          ...
          // If Firestore collection is empty, fall back to local or seed data
          const currentLocal = getLocal(collectionName);
          const dataToUse = currentLocal.length > 0 ? currentLocal : (DEFAULT_SEEDS[collectionName] || []);
          callback(sortFn ? [...dataToUse].sort(sortFn) : dataToUse);
        }
      },
      (err) => {
...
export async function fetchCollection(collectionName, sortFn = null) {
  let localItems = getLocal(collectionName);
  if (!localItems || localItems.length === 0) {
    localItems = DEFAULT_SEEDS[collectionName] || [];
  }
