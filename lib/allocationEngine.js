...
/**
 * Generates full monthly work allocation across all weeks belonging to the given monthKey.
 * 
 * @param {Object} params
 * @param {string} params.monthKey - Target month key (e.g. '2026-09')
 * @param {Array} params.workWeeks - All or filtered work week objects
 * @param {Array} params.clients - Active client records
 * @param {Array} params.employees - Active employee records
 * @param {Array} params.capacityRules - Capacity rules
 * @param {Array} params.workRequirements - Client work requirements (tagged by weekId or monthKey)
 * @param {Array} params.holidays - Holiday records
 * @param {Array} params.availabilityList - Employee availability overrides
 * @param {Array} params.existingAllocations - Pre-existing confirmed allocations
 * @returns {Object} Monthly allocation result containing allocations, surplus, employeeUtilization, weeklyBreakdown, etc.
 */
export function generateMonthlyAllocation({
  monthKey,
  workWeeks = [],
  clients = [],
  employees = [],
  ...
  // 1. Filter work weeks belonging to the month
  const targetWeeks = (workWeeks || []).filter((w) => {
    if (!monthKey || monthKey === 'all') return true;
    return (w.startDate || '').startsWith(monthKey) || w.monthKey === monthKey;
  }).sort((a, b) => (a.startDate || '').localeCompare(b.startDate || ''));

  if (targetWeeks.length === 0) {
    throw new Error(`No work weeks found for selected month (${monthKey || 'all'}). Please configure calendar weeks first.`);
  }
  ...
  let totalRequestedTasks = 0;
  let totalAllocatedTasks = 0;
  let totalSurplusTasks = 0;

  // 2. Iterate each week in the month and compute weekly allocation
  targetWeeks.forEach((week) => {
    // Requirements for this week
    const weekReqs = (workRequirements || []).filter((r) => r.weekId === week.id);

    // Week holidays
    const weekHolidays = Array.isArray(week.holidays) ? week.holidays : (holidays || []);

    // Existing allocations for this week
    const weekExistingAlloc = (existingAllocations || []).filter((a) => a.weekId === week.id);

    const weekResult = generateWeeklyAllocation({
      workWeek: week,
      clients,
      employees,
      capacityRules,
      ...
      availabilityList,
      existingAllocations: weekExistingAlloc
    });

    // Tag each allocation with monthKey and week details
    const taggedAlloc = (weekResult.allocations || []).map((a) => ({
      ...a,
      monthKey,
      weekName: week.name,
    ...
    const taggedSurplus = (weekResult.surplus || []).map((s) => ({
      ...s,
      monthKey,
      weekName: week.name,
    }));

    allAllocations.push(...taggedAlloc);
    allSurplus.push(...taggedSurplus);
    allWarnings.push(...(weekResult.warnings || []).map(w => ({ ...w, weekName: week.name })));

    totalRequestedTasks += (weekResult.validation?.totalRequestedTasks || 0);
    totalAllocatedTasks += (weekResult.validation?.totalAllocatedTasks || 0);
    totalSurplusTasks += (weekResult.validation?.totalSurplusTasks || 0);

    weeklyBreakdowns.push({
      week,
      weekId: week.id,
      weekName: week.name,
      allocations: taggedAlloc,
      surplus: taggedSurplus,
      employeeUtilization: weekResult.employeeUtilization,
      dailySchedules: weekResult.dailySchedules,
      clientSummary: weekResult.clientSummary,
    });
  });

  // 3. Aggregate Monthly Employee Utilization
  const employeeMonthlyMap = new Map();
  const activeEmployees = (employees || []).filter(e => e.status !== 'inactive');

  activeEmployees.forEach(emp => {
    employeeMonthlyMap.set(emp.id, {
      employeeId: emp.id,
      name: emp.name,
      role: emp.role,
      code: emp.employeeCode || '',
      monthlyTotalCapacityUnits: 0,
      monthlyUsedCapacityUnits: 0,
      monthlyAssignedPosts: 0,
      monthlyAssignedReels: 0,
      monthlyAssignedStories: 0,
      utilizationPercentage: 0,
      isOverCapacity: false,
    });
  });

  weeklyBreakdowns.forEach(wb => {
    Object.values(wb.employeeUtilization || {}).forEach(eu => {
      const empId = eu.empId || eu.employeeId;
      const state = employeeMonthlyMap.get(empId);
      if (state) {
        state.monthlyTotalCapacityUnits += (eu.totalCapacityUnits || eu.weeklyCapacityUnits || 0);
        state.monthlyUsedCapacityUnits += (eu.usedCapacityUnits || 0);
        state.monthlyAssignedPosts += (eu.assignedWork?.posts || 0);
        state.monthlyAssignedReels += (eu.assignedWork?.reels || 0);
        state.monthlyAssignedStories += (eu.assignedWork?.stories || 0);
      }
    });
  });

  for (const state of employeeMonthlyMap.values()) {
    state.utilizationPercentage = calculateUtilization(
      state.monthlyUsedCapacityUnits,
      state.monthlyTotalCapacityUnits
    );
    state.isOverCapacity = state.monthlyUsedCapacityUnits > state.monthlyTotalCapacityUnits;
  }

  // 4. Monthly Team Utilization
  let graphicMonthlyTotalCap = 0;
  let graphicMonthlyUsedCap = 0;
  let videoMonthlyTotalCap = 0;
  let videoMonthlyUsedCap = 0;

  for (const emp of employeeMonthlyMap.values()) {
    if (emp.role === ROLES.GRAPHIC_DESIGNER) {
      graphicMonthlyTotalCap += emp.monthlyTotalCapacityUnits;
      graphicMonthlyUsedCap += emp.monthlyUsedCapacityUnits;
    } else if (emp.role === ROLES.VIDEO_EDITOR) {
      videoMonthlyTotalCap += emp.monthlyTotalCapacityUnits;
      videoMonthlyUsedCap += emp.monthlyUsedCapacityUnits;
    }
  }

  // 5. Aggregate Monthly Client Deliverables Summary
  const clientMonthlySummary = {};
  (clients || []).forEach(c => {
    clientMonthlySummary[c.id] = {
      clientId: c.id,
      clientName: c.name,
      totalPosts: 0,
      totalReels: 0,
  ...
  allAllocations.forEach(a => {
    if (!clientMonthlySummary[a.clientId]) {
      clientMonthlySummary[a.clientId] = {
        clientId: a.clientId,
    ...
    const rec = clientMonthlySummary[a.clientId];
    rec.totalPosts += (Number(a.work?.posts) || 0);
    rec.totalReels += (Number(a.work?.reels) || 0);
    rec.totalStories += (Number(a.work?.stories) || 0);
  });

  Object.values(clientMonthlySummary).forEach(c => {
    c.totalDeliverables = c.totalPosts + c.totalReels + c.totalStories;
  });
  ...
  return {
    ...
    employeeUtilization: Array.from(employeeMonthlyMap.values()),
    clientSummary: clientMonthlySummary,
    weeklyBreakdowns,
    teamCapacity: {
      graphic: {
        totalUnits: graphicMonthlyTotalCap,
        usedUnits: graphicMonthlyUsedCap,
        utilization: calculateUtilization(graphicMonthlyUsedCap, graphicMonthlyTotalCap),
      },
      video: {
        totalUnits: videoMonthlyTotalCap,
        usedUnits: videoMonthlyUsedCap,
        utilization: calculateUtilization(videoMonthlyUsedCap, videoMonthlyTotalCap),
      },
    },
    validation: {
      totalRequestedTasks,
      totalAllocatedTasks,
      totalSurplusTasks,
      invariantSatisfied: Math.abs(totalRequestedTasks - (totalAllocatedTasks + totalSurplusTasks)) < EPSILON,
    },
    metadata: {
      generatedAt: new Date().toISOString(),
      monthKey,
      calculationVersion: '2.0-monthly',
    }
  };
}
