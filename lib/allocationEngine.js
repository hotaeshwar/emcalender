...
export function generateWeeklyAllocation({
  ...
  taskComponents.forEach(task => {
    ...
    const reqKey = `${clientId}_${contentType}_${roleComponent}`;
    ...
    const singleUnitCapacityWeight = convertTaskToCapacityUnits(contentType, roleComponent, 1, capacityRules);

    if (singleUnitCapacityWeight <= 0) {
      warnings.push({
        type: 'INVALID_WEIGHT',
        message: `Capacity rule weight for ${roleComponent} - ${contentType} is 0 or missing.`,
      });
    }

    // Step A: Find all eligible employees for this role
    const roleEmployees = Array.from(employeeCapacityMap.values()).filter(e => e.role === roleComponent);
    ...
    if (remainingQuantity > 0) {
      // If loop finished and items remain, send to surplus
      surplusList.push({
        clientId,
        weekId: workWeek.id || 'week_1',
        contentType,
  ...
  // Ensures clean consolidated records in Firestore
  ...
  surplusList.forEach(s => {
    ...
    if (!aggregatedSurplusMap.has(key)) {
      aggregatedSurplusMap.set(key, { ...s });
    } else {
...
export function generateDailySchedule(
  ...
  const dailySchedules = {}; // empId -> { [dateStr]: { posts, reels, stories, totalCapacityUnits, usedCapacityUnits } }
  ...
  allocations.forEach(alloc => {
    ...
    if (!dailySchedules[empId]) {
      ...
      effectiveWorkingDates.forEach(dateStr => {
        ...
        const avail = availabilityList.find(a => a.employeeId === empId && normalizeDate(a.date) === normalized);
    ...
    let dayIdx = 0;
    while ((postsToDistribute > 0 || reelsToDistribute > 0 || storiesToDistribute > 0) && dayIdx < availableDates.length * 10) {
