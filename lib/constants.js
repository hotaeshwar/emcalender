...
export function normalizeRole(roleStr, employeeCode = '') {
  const code = String(employeeCode || '').toUpperCase().trim();
  const s = String(roleStr || '').toLowerCase().trim().replace(/[\s_-]+/g, '');

  if (s.includes('video') || s.includes('edit') || s === 've' || s === 'editor' || s === 'vd') {
    return ROLES.VIDEO_EDITOR;
  }

  if (code.includes('VE') || code.includes('VD') || code.includes('EDIT')) {
    return ROLES.VIDEO_EDITOR;
  }

  if (s.includes('graphic') || s.includes('design') || s === 'gd' || s === 'designer') {
    return ROLES.GRAPHIC_DESIGNER;
  }

  if (code.includes('GD') || code.includes('DESIGN')) {
    return ROLES.GRAPHIC_DESIGNER;
  }

  return String(roleStr || '').toLowerCase().trim() || ROLES.GRAPHIC_DESIGNER;
}
