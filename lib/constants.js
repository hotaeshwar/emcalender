...
export function normalizeRole(roleStr) {
  if (!roleStr) return ROLES.GRAPHIC_DESIGNER;
  const s = String(roleStr).toLowerCase().trim().replace(/[\s_-]+/g, '');
  if (s.includes('video') || s.includes('edit')) return ROLES.VIDEO_EDITOR;
  if (s.includes('graphic') || s.includes('design')) return ROLES.GRAPHIC_DESIGNER;
  if (s === 've' || s === 'editor') return ROLES.VIDEO_EDITOR;
  if (s === 'gd' || s === 'designer') return ROLES.GRAPHIC_DESIGNER;
  return String(roleStr).toLowerCase().trim();
}
